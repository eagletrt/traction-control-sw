...
static void MATLABFunction(real_T rtu_new_sample, boolean_T rtu_external_reset,
  ...
  *rty_integral);
static void MATLABFunction_p(real_T rtu_newSample, real_T rtu_window_length,
  real_T *rty_mem);
...
void SLIP_step(RT_MODEL_SLIP *const SLIP_M)
{
   ...
   *  Sum: '<S3>/Add2'
   */
  Slip_estimation(0.203 * SLIP_in_omega_rl, SLIP_u + SLIP_yaw_rate * 0.605, 1.0,
                  &rtb_lambda);
   ...
   *  Product: '<S17>/Divide3'
   */
  MATLABFunction_p(rtb_error, SLIP_in_shallow_window_seconds /
  ...
  rtb_integral = SLIP_DW->DelayOneStep_DSTATE_e;
  ...
  rtb_mem_a = SLIP_DW->DelayOneStep_DSTATE_g;
   ...
   *  Product: '<S17>/Divide2'
   */
  MATLABFunction_p(rtb_error, SLIP_in_window_seconds /
                   SLIP_in_iteration_step_seconds, &rtb_mem_a);
   ...
   *  Sum: '<S17>/Sum'
   */
  rtb_derivative = (rtb_mem_a - rtb_derivative) /
   ...
   *  Sum: '<S17>/Sum2'
   */
  rtb_error = fmax((rtb_Switch2_j + rtb_integral) + rtb_derivative,
  ...
  SLIP_out_debug_bus_rl.filtered_lambda_error = rtb_mem_a;
   ...
   *  Sum: '<S2>/Add2'
   */
  Slip_estimation(0.203 * SLIP_in_omega_rr, SLIP_u + SLIP_yaw_rate * -0.605, 1.0,
                  &rtb_lambda);
   ...
   *  Switch: '<S9>/Switch'
   */
  MATLABFunction(rtb_Switch2_j * rtb_derivative, (SLIP_throttle < 0.03),
                 SLIP_in_minimum_torque, 100.0, SLIP_in_iteration_step_seconds,
                 &rtb_Product);

  /* Delay: '<S11>/Delay One Step' */
  rtb_mem_j = SLIP_DW->DelayOneStep_DSTATE_h;
   ...
   *  Product: '<S5>/Divide3'
   */
  MATLABFunction_p(rtb_Switch2_j, SLIP_in_shallow_window_seconds /
                   SLIP_in_iteration_step_seconds, &rtb_mem_j);

  /* Product: '<S12>/Product' incorporates:
   *  Inport: '<Root>/in_Kp'
   */
  rtb_error = rtb_mem_j * SLIP_in_Kp;

  /* Delay: '<S10>/Delay One Step' */
  rtb_mem_i = SLIP_DW->DelayOneStep_DSTATE_p;
   ...
   *  Product: '<S5>/Divide2'
   */
  MATLABFunction_p(rtb_Switch2_j, SLIP_in_window_seconds /
                   SLIP_in_iteration_step_seconds, &rtb_mem_i);

  /* Delay: '<S5>/Delay' incorporates:
   *  DataTypeConversion: '<S5>/Cast To Single'
   */
  if ((real32_T)rtb_derivative_tmp < 1.0F) {
    rtb_Switch2_j = rtb_mem_i;
  } else {
    if ((real32_T)rtb_derivative_tmp > 100.0F) {
      idxDelay = 100;
    } else {
   ...
   *  Sum: '<S5>/Sum'
   */
  rtb_derivative = (rtb_mem_i - rtb_Switch2_j) / in_differentiation_step_seconds
    * SLIP_in_Kd;
   ...
   *  Sum: '<S5>/Sum2'
   */
  rtb_Switch2_j = fmax((rtb_error + rtb_Product) + rtb_derivative,
                       SLIP_in_minimum_torque);
  ...
  SLIP_out_debug_bus_rr.filtered_lambda_error = rtb_mem_i;
  SLIP_out_debug_bus_rr.shallow_filtered_lambda_error = rtb_mem_j;
  ...
  /* Update for Delay: '<S17>/Delay' */
  SLIP_DW->Delay_DSTATE[99] = rtb_mem_a;

  /* Update for Delay: '<S5>/Delay' */
  SLIP_DW->Delay_DSTATE_m[99] = rtb_mem_i;
}
